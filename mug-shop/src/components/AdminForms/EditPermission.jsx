...
function EditPermission() {
    ...
    const get_permission = ()=>{
        Simple_get('https://hitmug.ir/api/permision/find/',true ,permission_id.id ,cookie.token ,'get' ,[])
            .then((d)=>{
                if (parseInt(d?.[2]) >= 200 && parseInt(d?.[2]) < 300) {
                    setSelectedPermission(d[0]?.name)
                } else {
                    Notifier('danger', 'خطا در دریافت دسترسی')
                }
            })
    }
    const submitForm = async () => {
        ...
        let data =
            await Simple_get('https://hitmug.ir/api/permision/update/', true, permission_id.id, cookie.token, 'patch', {...dataObj})
                .then((d => {
                        if (parseInt(d?.[2]) >= 200 && parseInt(d?.[2]) < 300) {
